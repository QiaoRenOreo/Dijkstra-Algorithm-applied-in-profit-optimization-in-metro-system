...
def distance(StartZone,EndZone,MetroStationLocation):
    a=(RowIndexZone(StartZone)-RowIndexZone(EndZone))**2+(ColumnIndexZone(StartZone)-ColumnIndexZone(EndZone))**2
    return math.sqrt(a)
...
def UpdateGraph(ZoneCode,graph,MetroStationLocation): #自变量是graph，return的还是graph，表示原graph被新的graph覆盖，且没有被保存。
...
def ShortestPathMatrix(MetroLine,MetroStationLocation):
    ...
    ShortestPathMatrix=[[-1 for OriginZoneCode in range(TotalAmountGrid)] for DestinationZoneCode in range(TotalAmountGrid)]
    for OriginZoneCode in range (1,TotalAmountGrid+1):
        for DestinationZoneCode in range (1,TotalAmountGrid+1):
...
def ImpedanceOfAllPaths(MetroLine,MetroStationLocation,MetroProperty):
    ImpedanceMatrix=[[float("inf") for OriginZoneCode in range(TotalAmountGrid)] for DestinationZoneCode in range(TotalAmountGrid)]
    PathMatrix=ShortestPathMatrix(MetroLine,MetroStationLocation)
    for RowIndex in range (0,TotalAmountGrid):#(0,TotalAmountGrid)
        for ColumnIndex in range (0,TotalAmountGrid):#(0,TotalAmountGrid)
...
def DeterrenceMatrix(MetroLine,MetroStationLocation,MetroProperty):    
    DeterMatrix=[[float("inf") for i in range(TotalAmountGrid)] for j in range(TotalAmountGrid)] # initialize the deterrence list. In F(C), F is the deterrence function
    Impedance=ImpedanceOfAllPaths(MetroLine,MetroStationLocation,MetroProperty)    
    for i in range(0,TotalAmountGrid):
        for j in range(0,TotalAmountGrid): 
            if Impedance[i][j]!=float("inf"):
                DeterMatrix[i][j]=1000*math.exp(-0.1*Impedance[i][j]) #give deterrence function. question!!! exp() is so small that 0.0000003=0 
    return DeterMatrix

def SumProduction(T): 
    SumOriginList=[-1 for i in range (TotalAmountGrid)] #initialize SumOriginList.    
    for i in range (0,TotalAmountGrid):
        SumOriginList[i]=sum(T[i])
    return SumOriginList

def SumAttraction(T):
    SumDestinationList=[-1 for i in range (TotalAmountGrid)] #initialize SumDestinationList.       
    for j in range (0,TotalAmountGrid):
        column=j
...
def TripDistribution(TotalAmountGrid,MetroLine,SocialEcoFactorsList,SocialEcoCoeffList,MetroStationLocation,MetroProperty):
    T=[[-1 for i in range(TotalAmountGrid)] for j in range(TotalAmountGrid)] #initialize T (number of trips going from origin i to destination j)    
    Coefa=[1 for i in range (TotalAmountGrid)] #initialize coefficient a
    Coefb=[1 for j in range (TotalAmountGrid)] #initialize coefficient b
    DeterMatrix=DeterrenceMatrix(MetroLine,MetroStationLocation,MetroProperty)
    
    for i in range(0,TotalAmountGrid): #initial T
        for j in range(0,TotalAmountGrid):
            if DeterMatrix[i][j]!=float("inf"):# not infinity
                T[i][j]=1*1*(DeterMatrix[i][j])
            else:
                T[i][j]=0
    
    TargetAttra=TargetAttractionList(SocialEcoFactorsList,SocialEcoCoeffList)
    TargetProd=TargetProductionList(SocialEcoFactorsList,SocialEcoCoeffList)
    IterationTime=1 
  
    while IterationTime<=1000:#unsolved. or AccuracyDestinations(MetroLine,T,SocialEcoFactorsList,SocialEcoCoeffList)==True: #and AccuracyDestinations(MetroLine,T)==True):      
        SumColumn=SumAttraction(T)
        for j in range (0,TotalAmountGrid): ########### first part of the loop: correct attractions                       
            Coefb[j]=Coefb[j]*(float(TargetAttra[j])/SumColumn[j]) # print "Coefb is", Coefb     
        for a in range(0,TotalAmountGrid):
            for b in range(0,TotalAmountGrid):
                if DeterMatrix[a][b]!=float("inf"): 
                    T[a][b]=Coefa[a]*Coefb[b]*DeterMatrix[a][b]#Coefa[a]*Coefb[b]*T[a][b] #       

        SumRow=SumProduction(T)
        for i in range (0,TotalAmountGrid): ########### first part of the loop: correct productions                                                 
            Coefa[i]=Coefa[i]*(float(TargetProd[i])/SumRow[i]) #Coefb[j]*float()!!!!! print "Coefb is", Coefb     
        for p in range(0,TotalAmountGrid):
            for q in range(0,TotalAmountGrid):
                if DeterMatrix[a][b]!=float("inf"): 
                    T[p][q]=Coefa[p]*Coefb[q]*(DeterMatrix[p][q])#Coefa[p]*Coefb[q]*T[p][q]#Coefa[p]*Coefb[q]*(DeterMatrix[p][q])
...
def FillInTrafficVolume(MetroLine,MetroStationLocation,MetroProperty):
    TrafficVolume=InitialTrafficVolumeOfAllLines(MetroLine,MetroStationLocation)
    PathMatrix=ShortestPathMatrix(MetroLine,MetroStationLocation)
    TripEndMatrix=TripDistribution(TotalAmountGrid,MetroLine,SocialEcoFactorsList,SocialEcoCoeffList,MetroStationLocation,MetroProperty)
    for m in range (0,TotalAmountGrid):
        for n in range (0,TotalAmountGrid):
            path=PathMatrix[m][n]
            if path!=0:
                    ...
                    for x in range (0,len(EdgeLineCodePair)):
                        ...
                        for y in range (0,len(TrafficVolume[LineCode-1])):
                            if TrafficVolume[LineCode-1][y][0]==EdgeLineCodePair[x][0]:# and TrafficVolume[i][1]==EdgeLineCodePair[i][1]:                                                    
                                TrafficVolume[LineCode-1][y][2]=TrafficVolume[LineCode-1][y][2]+TripEndMatrix[m][n]
...
def CapacityVolumeComparison(MetroLine,MetroProperty,MetroStationLocation):
    List=FillInTrafficVolume(MetroLine,MetroStationLocation,MetroProperty)
...
###################################3
def RevenueOfSingleLine(LineCode,MetroLine,MetroProperty,MetroStationLocation):
    TrafficVolume=FillInTrafficVolume(MetroLine,MetroStationLocation,MetroProperty)
...
def RevenueOfAllLines(MetroLine,MetroProperty,MetroStationLocation):
    revenue=0
    for LineCode in range (1,len(MetroLine)+1):
        revenue=revenue+RevenueOfSingleLine(LineCode,MetroLine,MetroProperty,MetroStationLocation)
    return revenue
...
def IncomeOfSingleLine(LineCode,MetroStationLocation,MetroLine,BudgetData,MetroProperty):
    OperationCost=OperationCostOfSingleLine(LineCode,MetroStationLocation,MetroLine,BudgetData,MetroProperty)
    Revenue=RevenueOfSingleLine(LineCode,MetroLine,MetroProperty,MetroStationLocation)
    Income=Revenue-OperationCost
    return Income
    
def IncomeOfAllLines(MetroStationLocation,MetroLine,BudgetData,MetroProperty):
    TotalIncome=0
    for LineCode in range (1,len(MetroLine)+1):
       TotalIncome=TotalIncome+IncomeOfSingleLine(LineCode,MetroStationLocation,MetroLine,BudgetData,MetroProperty) 
    return TotalIncome

def RunScenario(ScenarioCode): 
    FileMetroStation=RootPath+r"\Scenario%02d\MetroStation%02d.csv"%(ScenarioCode,ScenarioCode)
    #FileMetroStation=r"D:\2015 Qiao\UT ATLAS study\semester 4\PP\MyPthon\MetroData\Scenario%02d\MetroStation%02d.csv"%(ScenarioCode,ScenarioCode)    
    MetroStationLocation=IntegerFile(FileMetroStation) 
    #print MetroStationLocation
    
    FileMetroLine=FileMetroStation=RootPath+r"\Scenario%02d\MetroLine%02d.csv"%(ScenarioCode,ScenarioCode)     
    #FileMetroLine=FileMetroStation=r"D:\2015 Qiao\UT ATLAS study\semester 4\PP\MyPthon\MetroData\Scenario%02d\MetroLine%02d.csv"%(ScenarioCode,ScenarioCode)     
    MetroLine=IntegerFile(FileMetroLine)# number of "station" text should be no more than the max amount of stations in a line
    #print MetroLine
    FileMetroProperty=RootPath+r"\Scenario%02d\MetroProperty%02d.csv"%(ScenarioCode,ScenarioCode)
    #FileMetroProperty=r"D:\2015 Qiao\UT ATLAS study\semester 4\PP\MyPthon\MetroData\Scenario%02d\MetroProperty%02d.csv"%(ScenarioCode,ScenarioCode)
    MetroProperty=FloatFile(FileMetroProperty)

    outcsvfile=RootPath+r"\Output%02d\output%02d.csv"%(ScenarioCode,ScenarioCode)
    #outcsvfile=r"D:\2015 Qiao\UT ATLAS study\semester 4\PP\MyPthon\MetroData\Output%02d\output%02d.csv"%(ScenarioCode,ScenarioCode)
    out_file=open(outcsvfile,'w')
    lineout='Output Of Scenarial %02d\n'%(ScenarioCode) #title of the output file
    ...
    out_file.write (lineout)
    TripEnd=TripDistribution(TotalAmountGrid,MetroLine,SocialEcoFactorsList,SocialEcoCoeffList,MetroStationLocation,MetroProperty)    
    for a in range (0,len(TripEnd)): # in the 2nd line, write the name of all ZoneCodes
        lineout='Zone%d,'%(a+1)
        out_file.write (lineout)
    lineout='Trip Production Of Each Zone\n' #go to next row
    out_file.write (lineout)
    
    SumProd=SumProduction(TripEnd)
    for i in range (0,len(TripEnd)): #in the 3rd line, a Zonecode+a row of impedance value
        lineout='Zone%d,'%(i+1)
        out_file.write (lineout)
        for j in range (0,len(TripEnd)):
            lineout='%10.5f,'%(TripEnd[i][j])        
            out_file.write (lineout)
        lineout='%10.5f\n'%(SumProd[i])
        out_file.write (lineout)
    ...
    out_file.write (lineout)    
    SumAttra=SumAttraction(TripEnd)
    for m in range (0,len(SumAttra)):
        lineout='%10.5f,'%(SumAttra[m])        
        out_file.write (lineout)    
    ...
    for LineCode in range (1,len(MetroLine)+1):
        ...
        ReveneSingle=RevenueOfSingleLine(LineCode,MetroLine,MetroProperty,MetroStationLocation)
        IncomeSingle=IncomeOfSingleLine(LineCode,MetroStationLocation,MetroLine,BudgetData,MetroProperty)
        lineout= '%d,%10.5f,%10.5f,%10.5f\n'%(LineCode,OperCostSingle,ReveneSingle,IncomeSingle)
        out_file.write (lineout)
    
    RAll=RevenueOfAllLines(MetroLine,MetroProperty,MetroStationLocation)
    OCAll=OperationCostOfAllLines(MetroStationLocation,MetroLine,BudgetData,MetroProperty)
    IAll=IncomeOfAllLines (MetroStationLocation,MetroLine,BudgetData,MetroProperty)
    lineout= 'Sum,%10.5f,%10.5f,%10.5f\n'%(OCAll,RAll,IAll)
    out_file.write (lineout)
    out_file.flush()
...
GridSize=5 #ZoneCodeList=[[-1 for i in range(GridSize)] for j in range(GridSize)] 
TotalAmountGrid=GridSize**2

RootPath=r"D:\zl\Python\MetroPy\MetroData"
FileSocialEcoFactors=RootPath+r"\CaseData\SocialEcoFactors.csv" # input Factor File
#FileSocialEcoFactors=r"D:\2015 Qiao\UT ATLAS study\semester 4\PP\MyPthon\MetroData\CaseData\SocialEcoFactors.csv" # input Factor File
SocialEcoFactorsList=FloatFile(FileSocialEcoFactors) #read the factor file, convert all strings to float, build up a list
FileSocialEcoCoeff=RootPath+r"\CaseData\CoeffSocialEcoFactors.csv" #input CoeffFile
#FileSocialEcoCoeff=r"D:\2015 Qiao\UT ATLAS study\semester 4\PP\MyPthon\MetroData\CaseData\CoeffSocialEcoFactors.csv" #input CoeffFile
...
FileImpedanceCalculationFactors=RootPath+r"\CaseData\ImpedanceCalculationFactors.csv"
#FileImpedanceCalculationFactors=r"D:\2015 Qiao\UT ATLAS study\semester 4\PP\MyPthon\MetroData\CaseData\ImpedanceCalculationFactors.csv"
ImpedanceCalculationFactors=FloatFile(FileImpedanceCalculationFactors)
FileBudgetData=RootPath+r"\CaseData\BudgetData.csv"
#FileBudgetData=r"D:\2015 Qiao\UT ATLAS study\semester 4\PP\MyPthon\MetroData\CaseData\BudgetData.csv"
